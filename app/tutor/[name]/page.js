...
const page = ({ params }) => {
  ...
  const handleSubmit = (e) => {
    ...
    const inputPrompt = `I will give you an English statement to convert to a JSON object in this format: availability: [{dayOfWeek: { type: String}, timeSlots: [{from: { type: String }, to: { type: String },},],},]. Example: {"availability": [{"dayOfWeek": "monday", "timeSlots": [{ "from": "09:00", "to": "12:00" }]},{"dayOfWeek": "tuesday", "timeSlots": [{ "from": "14:00", "to": "17:00" }]},{"dayOfWeek": "friday", "timeSlots": [{ "from": "10:00", "to": "12:00" },{ "from": "1:00", "to": "2:00" }]}]}.This examples means that it is available on monday from 09:00 till 12:00 and from 14:00 till 17:00 on tuesday and from 10:00 till 12:00 on friday and from 01:00 till 02:00 till friday. There must be at least 1 in timeSlots for every day you provide. dayOfWeek must be saturday, sunday, monday, tuesday, wednesday, thursday, or friday. timeSlots from and to must be between 00:00 and 23:59. If the dayOfWeek doesn't have any timeSlots, don't provide it. Convert the time given in the English statement to a 24-hour clock format. from is less than to. from must be greater than or equal to 00:00. to must be less than or equal to 23:59. timeSlots must contain at least 1 time. If the English statement does not contain information relevant to converting return 1 random time at a random day. Provide JSON object only starting with '{' and ending with '}'.For example if the English statement is (I am available between noon and 4pm on weekends, after 7 pm to midnight on Monday and Wednesday, and after 9pm otherwise), the output should be {"availability": [{"dayOfWeek": "saturday","timeSlots": [{"from": "12:00","to": "16:00"}]},{"dayOfWeek": "sunday","timeSlots": [{"from": "12:00","to": "16:00" }]},{"dayOfWeek": "monday","timeSlots": [ {"from":"19:00","to": "23:59"}]},{"dayOfWeek": "wednesday","timeSlots": [{"from": "19:00","to": "23:59" }]},{"dayOfWeek": "tuesday","timeSlots": [{"from": "21:00","to": "23:59"}]},{"dayOfWeek": "thursday","timeSlots": [{"from": "21:00","to": "23:59"}]},{"dayOfWeek": "friday","timeSlots": [{"from": "21:00","to": "23:59"}]}]}. Here is the English statement to convert: “${text}".`;
    fetch(apiUrl, {
      method: "POST",
      headers: {
